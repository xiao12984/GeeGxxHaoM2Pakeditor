...
- 打开并校验 GEEPAK3 归档。
- 打开传统 `.wzl` + 同名 `.wzx` 资源，浏览、搜索、预览并导出 PNG；项目自建 WZL 可写回，xiami M2Zip 原始资源只读。
- 按逻辑索引浏览、搜索和预览图片。
- 解码固定调色板、R5G6B5、RGB24、RGB24+A8、XRGB32 和 ARGB32。
- 新建 PAK 或 WZL/WZX 资源文件。
- 导入、替换、删除图片，导出为 PNG。
- 修改图片 X/Y 绘制偏移。
- 保存或另存为 GEEPAK3 与项目自建 WZL/WZX，按格式重新生成索引和图片块头。
...
- 项目自建 WZL/WZX 使用独立明文索引写回链路，不走 GEEPAK3 加密索引链路；xiami M2Zip 原始资源当前只读。
