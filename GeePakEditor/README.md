...
- 打开并校验 GEEPAK3。
- 打开传统 `.wzl` + 同名 `.wzx` 资源，可浏览、预览和导出 PNG；项目自建 WZL 可写回，xiami M2Zip 原始资源只读。
- 按逻辑索引浏览、搜索和预览图片。
- 解码调色板、R5G6B5、RGB24、RGB24+A8、XRGB32、ARGB32。
- 新建 PAK 或 WZL/WZX 资源文件。
- 导入、替换、删除、导出 PNG。
- 修改 X/Y 偏移并保存或另存为 PAK 或项目自建 WZL/WZX。
- 自动读取 `FilePassword.txt` 的 `完整路径|密码` 记录。
- 未修改图片保持原始载荷，新增和替换图片写为 ARGB32。
- 项目自建 WZL/WZX 支持导入、替换、删除、坐标修改和写回；xiami M2Zip 支持只读预览与导出。
